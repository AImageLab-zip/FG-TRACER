...
def pil_collate_fn(batch):
    image, question, image_id, answer_tokens = zip(*batch)  # Assuming each item is a tuple (PIL.Image, label)
    return list(image), list(question), list(image_id), list(answer_tokens)

class ChartQADataset(Dataset):
    def __init__(self, image_dir = '/path/to/ChartQA/ChartQA Dataset/test/png', data_path = "LLaVA/tracing_information_flow/datasets/chartqa/chartqa_dataset_correct_answers_complete.json") -> None:
        super().__init__() 
    ...
    def __getitem__(self, index):
        ...
        return image, question, image_id, answer_tokens


def parse_args():
    parser = argparse.ArgumentParser(description="ChartQA Eval")
    parser.add_argument(
        "--answer_path", type=str, default="LLaVA/results/chartqa"
    )
    parser.add_argument(
        "--data_path", type=str, default="LLaVA/tracing_information_flow/datasets/chartqa/chartqa_dataset_correct_answers_complete.json"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    dataset = ChartQADataset(data_path=args.data_path, image_dir=args.image_dir) 
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            images, questions, image_ids, answer_tokens = data
            image = images[0]
            ...
            prob_layers = model.generate_multimodal_with_attention_blocking(prompt=message, answer_tokens=answer_tokens, image=image, max_gen_len=1024, block_types=args.block_types, k=args.k)
